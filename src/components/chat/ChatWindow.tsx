...
export function ChatWindow({ conversationId }: Props) {
  ...
  useEffect(() => {
    function handleToolSelect(e: CustomEvent) {
      ...
      switch (tool) {
        case 'search':
          setWebSearch(w => !w); setDeepResearch(false); setRightPanel(null); break;
        case 'research':
          setDeepResearch(d => !d); setWebSearch(false); setRightPanel(null); break;
        case 'image':
          setRightPanel(p => p === 'image' ? null : 'image'); setActiveArtifact(null); break;
        case 'project':
          setRightPanel(p => p === 'project' ? null : 'project'); setActiveArtifact(null); break;
        case 'canvas':
          setRightPanel(p => p === 'canvas' ? null : 'canvas'); setActiveArtifact(null); break;
        case 'execute':
          setRightPanel(p => p === 'execute' ? null : 'execute'); setActiveArtifact(null); break;
        case 'plugins':
          setPlugins(p => !p); break;
      }
    }
    window.addEventListener('aria:tool-select', handleToolSelect as EventListener);
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.done) {
              ...
              } else if (mode === 'builder' && activeArtifact) {
      ...
      <div className={`flex flex-col min-w-0 overflow-hidden transition-all duration-200
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-white/5 bg-[#16161d] flex-shrink-0">
          ...
          <div className="flex gap-1 flex-wrap flex-1 min-w-0">
            ...
            {deepResearch && <span className="text-[10px] bg-purple-500/20 text-purple-300 border border-purple-500/30 px-2 py-0.5 rounded-full">🔬 Research on</span>}
            {!plugins && <span className="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded-full">Plugins off</span>}
          </div>
        </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-center gap-3 pb-8 px-2">
              ...
              <div>
                ...
                <p className="text-[#888899] text-xs mt-1 max-w-[260px]">
                  {mode === 'builder' ? "Describe what to build — live preview opens automatically." : 'Your AI assistant. Pick a tool from the sidebar.'}
                </p>
              </div>
