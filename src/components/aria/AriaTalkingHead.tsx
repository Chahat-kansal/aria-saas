...
export function AriaTalkingHead({ isActive, responseText }: AriaTalkingHeadProps) {
  ...
  // Load TalkingHead from CDN — importmap in layout.tsx resolves 'three' for us
  useEffect(() => {
  ...
  // Initialise avatar once script + container ready
  useEffect(() => {
  ...
  // Silent visual lip sync — volume 0 drives mouth shapes only
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {/* Sound bars — visible when speaking */}
      {isActive && loaded && (
