...
export default function ActionPreviewCard({ action, onConfirm, onCancel, loading, proposePlan, onSavePlan }: Props) {
  return (
    <div className="mt-2 rounded-xl p-4 space-y-3"
          ...
          <>
            ...
            {onSavePlan && (
              <button
                ...
                style={{ background: 'rgba(45,82,64,0.15)', border: '1px solid rgba(127,184,151,0.25)', color: 'rgba(127,184,151,0.8)' }}
              >
                Save plan
              </button>
            )}
      ...
      <p className="text-[10px] text-center" style={{ color: 'rgba(255,255,255,0.25)' }}>
        {proposePlan ? 'This action requires manual review — save it to execute when ready' : 'Or type "yes" / "confirm" to proceed · "no" / "cancel" to abort'}
      </p>
    </div>
  )
}
