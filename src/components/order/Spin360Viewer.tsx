...
export function Spin360Viewer({ slug, sizeScale = 1.0, size = 320 }: Spin360ViewerProps) {
  ...
  // All mutable state lives in refs so animation loop never re-closes
  const imagesRef = useRef<HTMLImageElement[]>([])
  const frameRef = useRef(0)          // float: sub-frame for smooth momentum
  ...
  // Effect 1: preload all 24 frames whenever slug changes
  useEffect(() => {
    imagesRef.current = []
    setLoadedCount(0)
    setHintVisible(true)
  ...
  // Effect 2: animation loop + pointer drag
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Hint badge — hides on first drag */}
