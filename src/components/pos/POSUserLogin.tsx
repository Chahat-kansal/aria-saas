...
export function POSUserLogin({ businessId, businessName, onLogin, onSkip }: Props) {
  ...
  if (users.length === 0) return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg-base)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center', fontFamily: "'Manrope',system-ui,sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 32 }}>{businessName}</p>
        <div style={{ background: 'rgba(26,23,40,0.8)', backdropFilter: 'blur(24px)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 20, padding: '28px 32px', maxWidth: 380, width: '100%' }}>
          <p style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>Set up your first cashier</p>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 24 }}>Add yourself as a cashier to start using the register</p>
          ...
          <button onClick={onSkip} style={{ marginTop: 12, width: '100%', background: 'none', border: 'none', color: 'var(--text-tertiary)', fontSize: 12, cursor: 'pointer', padding: '8px 0' }}>
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg-base)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, fontFamily: "'Manrope',system-ui,sans-serif", position: 'relative', overflow: 'hidden' }}>
      <Orb style={{ width: 400, height: 400, top: '-100px', left: '-100px', background: 'radial-gradient(circle,rgba(139,92,246,0.15),transparent 70%)', filter: 'blur(80px)', animation: 'orb-pulse-0 4s ease-in-out infinite' }} />
              ...
              {users.map((u, idx) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: 'var(--bg-elevated)', border: '1px solid #2A2540', borderRadius: 16,
                    ...
                    onMouseLeave={e => { const el = e.currentTarget; el.style.border = '1px solid #2A2540'; el.style.background = 'var(--bg-elevated)'; }}
                  >
                    ...
                    <span style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-tertiary)', fontWeight: 600 }}>{u.role}</span>
                  </button>
                );
              })}
            </div>
            <div style={{ textAlign: 'center' }}>
              <button onClick={onSkip} style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', fontSize: 12, cursor: 'pointer', padding: '8px 0' }}>
          ...
          <div style={{ width: '100%', maxWidth: 300, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 24 }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{selectedUser.name}</p>
                <p style={{ fontSize: 11, color: 'var(--text-tertiary)', textTransform: 'capitalize' }}>{selectedUser.role}</p>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 20, animation: shaking ? 'shake 0.4s ease' : 'none' }}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ width: 14, height: 14, borderRadius: '50%', background: pin.length > i ? '#8B5CF6' : 'transparent', border: `2px solid ${pin.length > i ? '#8B5CF6' : 'transparent'}`, transition: 'all 150ms ease', transform: pin.length > i ? 'scale(1.1)' : 'scale(1)' }} />
              ))}
            </div>
              ...
              {NUM_PAD.map((k, i) => (
                <button key={i}
                  ...
                  style={{
                    ...
                    background: k ? 'var(--bg-elevated)' : 'transparent',
                    border: k ? '1px solid #2A2540' : 'none',
                    color: 'var(--text-primary)', fontSize: 18, fontWeight: 700, cursor: k ? 'pointer' : 'default',
                  ...
                  onMouseLeave={e => { if (k) { const el = e.currentTarget; el.style.border = '1px solid #2A2540'; el.style.background = 'var(--bg-elevated)'; }}}
