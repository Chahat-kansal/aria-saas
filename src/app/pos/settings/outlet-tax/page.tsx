...
export default function OutletTaxPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-xl font-semibold mb-2">Per-outlet tax overrides</h1>
      <p className="text-xs text-[rgba(26,26,22,.5)] mb-6">Override the base tax rate for specific outlets. Useful for multi-state retailers (NSW vs VIC) or international (AU vs NZ).</p>
      <div className="space-y-6">
        {outlets.map(o => (
          <div key={o.id} className="bg-white rounded-2xl border p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-sm">{o.name}</h2>
              <span className="text-xs text-[rgba(26,26,22,.4)]">{o.state_code ?? o.tax_jurisdiction ?? 'AU'}</span>
            </div>
            <table className="w-full text-sm">
              <thead className="text-xs text-[rgba(26,26,22,.5)]">
                <tr>
        ...
        {outlets.length === 0 && (
          <div className="bg-white rounded-2xl border p-8 text-center text-sm text-[rgba(26,26,22,.4)]">No outlets found. Create outlets first at Settings → Registers & Outlets.</div>
        )}
      </div>
    </div>
  )
