...
export default function ExpoPage() {
  ...
  const reload = async () => {
    ...
    if (r.ok) { const j = await r.json(); setTickets((j.tickets ?? []) as TicketWithProduct[]) }
    setLoading(false)
  }
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-page, #0E1411)', color: 'var(--text-primary, #E8EDE7)' }}>
      ...
      <div className="p-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          {Object.keys(byTable).length === 0 && <div className="text-center py-12" style={{ color: 'var(--text-secondary, #A8B5A8)' }}>All caught up.</div>}
        </div>
        <div>
