...
export default function ReelStudioPage() {
  ...
  async function loadBiz(uid: string) {
    try {
      ...
      const { data: bizList } = await supabase.from('businesses').select('id')
        .eq('user_id', uid).eq('is_active', true).order('created_at', { ascending: false }).limit(10)
  ...
  function applyIdea(idea: ReelIdea) {
    ...
    const tags = (idea.hashtags ?? []).map((h: string) => '#' + h).join(' ')
  ...
  async function generate() {
    ...
    if (clips === 1) {
      ...
      try {
        const res = await fetch(EDGE, {
          ...
          body: JSON.stringify({ ...basePayload, prompt: prompt || null, duration_seconds: duration }),
        })
        const d = await res.json()
    ...
    // Multi-clip
    setGenMsg('🎬 Generating ' + clips + ' clips for ' + duration + 's reel… (~' + (clips * 90) + 's total)')
    try {
      const clipUrls: string[] = []
      for (let i = 0; i < clips; i++) {
        const clipPrompt = prompt
          ? (i === 0 ? prompt : prompt + ', continuation scene ' + (i + 1) + ' of ' + clips)
          : null
        const res = await fetch(EDGE, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + userToken },
          body: JSON.stringify({ ...basePayload, prompt: clipPrompt, duration_seconds: clipSecs }),
        })
        const d = await res.json()
        if (!d.job_id) throw new Error('Clip ' + (i + 1) + ' failed: ' + (d.error ?? 'No job_id'))
        setGenMsg('⏳ Clip ' + (i + 1) + '/' + clips + ' submitted, waiting…')
        setGenProgress(10 + Math.floor(i * 70 / clips))
        const url = await pollClip(d.job_id, d.session_id, userToken)
        if (!url) throw new Error('Clip ' + (i + 1) + ' generation failed')
        clipUrls.push(url)
        setClipProgress(i + 1)
        setGenProgress(10 + Math.floor((i + 1) * 70 / clips))
      }
      ...
      if (clipUrls.length === 1) {
        ...
        setStitching(true)
        setGenMsg('🎞 Stitching ' + clipUrls.length + ' clips…')
        // Simple approach: use first clip URL, user can download each
        // Full ffmpeg stitching would require @ffmpeg/ffmpeg package
        setLatestVideo(clipUrls[0])
        setStitching(false)
        setGenMsg('✅ ' + clipUrls.length + ' clips ready! Showing clip 1. Download all below.')
      }

      setGenerating(false); setGenProgress(100); setTab('edit')
      if (bid) loadBiz(bid)
    } catch (e: any) {
      setGenMsg('❌ ' + e.message); setGenerating(false); setGenProgress(0); setStitching(false)
    }
  }
  ...
  const S = {
    ...
    left: { borderRight: '1px solid rgba(255,255,255,0.06)', overflowY: 'auto' as const, padding: '20px 16px', display: 'flex', flexDirection: 'column' as const, gap: 14 },
    ...
    card: { background: 'rgba(255,255,255,0.025)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: '14px 16px' },
    label: { fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', textTransform: 'uppercase' as const, marginBottom: 10, display: 'block' },
  ...
  if (loading) return (
    <div style={{ ...S.page, alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 40, height: 40, border: '3px solid rgba(127,184,151,0.2)', borderTopColor: '#7FB897', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
  ...
  return (
    <div style={S.page}>
      ...
      {tab === 'create' && (
        <div style={S.body}>
          <div style={S.left}>
            ...
            {/* Aria Ideas */}
            <div style={{ background: 'linear-gradient(135deg,rgba(127,184,151,0.08),rgba(45,82,64,0.12))', border: '1px solid rgba(127,184,151,0.2)', borderRadius: 14, padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: ideasLoaded ? 12 : 0 }}>
                ...
                <button onClick={loadIdeas} disabled={ideasLoading} style={{ fontSize: 10, padding: '4px 10px', borderRadius: 99, border: 'none', cursor: ideasLoading ? 'wait' : 'pointer', background: 'rgba(127,184,151,0.15)', color: '#7FB897', fontFamily: 'inherit', fontWeight: 700 }}>
                  {ideasLoading ? '⏳' : ideasLoaded ? '↻ Refresh' : '✦ Generate'}
                </button>
              </div>
              {ideasLoading && <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', paddingTop: 8 }}>Reading your sales data…</div>}
              {ideasLoaded && reelIdeas.map((idea, i) => (
                <div key={i} style={{ marginBottom: 6 }}>
                  <div onClick={() => setExpandedIdea(expandedIdea === i ? null : i)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', borderRadius: 10, cursor: 'pointer', background: expandedIdea === i ? 'rgba(127,184,151,0.12)' : 'rgba(255,255,255,0.04)', boxShadow: expandedIdea === i ? 'inset 0 0 0 1.5px rgba(127,184,151,0.4)' : 'inset 0 0 0 1px rgba(255,255,255,0.07)', transition: 'all 120ms' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 99, fontWeight: 700, background: idea.urgency === 'high' ? 'rgba(239,68,68,0.2)' : idea.urgency === 'medium' ? 'rgba(245,158,11,0.2)' : 'rgba(127,184,151,0.2)', color: idea.urgency === 'high' ? '#EF4444' : idea.urgency === 'medium' ? '#F59E0B' : '#7FB897' }}>
                  ...
                  {expandedIdea === i && (
                    <div style={{ padding: '10px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: '0 0 10px 10px', border: '1px solid rgba(127,184,151,0.15)', borderTop: 'none' }}>
                      <div style={{ fontSize: 11, color: '#7FB897', marginBottom: 6, lineHeight: 1.5 }}>📊 {idea.why}</div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', marginBottom: 8, lineHeight: 1.5 }}>🎬 {idea.prompt}</div>
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginBottom: 10 }}>Hook: &quot;{idea.hook}&quot;</div>
                      <button onClick={() => applyIdea(idea)} style={{ width: '100%', padding: '9px 0', borderRadius: 9, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, fontWeight: 800, background: 'linear-gradient(135deg,#7FB897,#2D5240)', color: '#fff' }}>✦ Use this idea →</button>
                    </div>
                  )}
                </div>
              ))}
            ...
            {/* Influencer */}
            <div style={S.card}>
              <span style={S.label}>AI Influencer {selectedInf?.soul_status === 'ready' && <span style={{ color: '#7FB897', marginLeft: 6 }}>✓ Soul trained</span>}</span>
              {selectedInf ? (
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, marginTop: 2 }}>{selectedInf.soul_status === 'ready' ? '✓ Consistent face' : 'Reference frame'}</div>
                  </div>
                  <button onClick={() => setShowPicker(true)} style={S.chip(false)}>Change</button>
                </div>
              ) : (
                <button onClick={() => setShowPicker(true)} style={{ width: '100%', padding: 12, borderRadius: 10, border: '1.5px dashed rgba(127,184,151,0.25)', background: 'transparent', cursor: 'pointer', color: 'rgba(255,255,255,0.4)', fontFamily: 'inherit', fontSize: 12 }}>
            ...
            <div style={S.card}>
              ...
              {scenePreview ? (
                <div style={{ position: 'relative', display: 'inline-block' }}>
                  ...
                  {sceneUploading && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11 }}>Uploading…</div>}
                ...
                <button onClick={() => sceneRef.current?.click()} style={{ width: '100%', padding: 10, borderRadius: 8, border: '1px dashed rgba(255,255,255,0.1)', background: 'transparent', cursor: 'pointer', color: 'rgba(255,255,255,0.35)', fontFamily: 'inherit', fontSize: 12 }}>📷 Upload product/shop photo</button>
              )}
            </div>
            ...
            <div style={S.card}>
              ...
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={3} placeholder="e.g. Walks into a warm café, picks up a flat white, smiles at camera" style={{ width: '100%', padding: '10px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontSize: 12, fontFamily: 'inherit', resize: 'none', outline: 'none', boxSizing: 'border-box', lineHeight: 1.6 }} />
            </div>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                ...
                <input type="number" min={3} max={60} placeholder="sec" onChange={(e) => { const v = parseInt(e.target.value); if (!isNaN(v) && v >= 3 && v <= 60) setDuration(v) }} style={{ width: 60, padding: '6px 8px', borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 12, fontFamily: 'inherit', outline: 'none' }} />
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)' }}>3–60s</span>
              </div>
              <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                {RESOLUTIONS.map((r) => (<button key={r.id} onClick={() => setResolution(r.id)} style={S.chip(resolution === r.id)}>{r.label} · {r.note}</button>))}
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              <div style={S.chipRow}>
                {GENRES.map((g) => (<button key={g} onClick={() => setGenre(g)} style={S.chip(genre === g)}>{g.charAt(0).toUpperCase() + g.slice(1)}</button>))}
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(127,184,151,0.05)', border: '1px solid rgba(127,184,151,0.15)', borderRadius: 12, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', marginTop: 2 }}>{resolution} · {duration}s{clipsNeeded(duration) > 1 ? ' · ' + clipsNeeded(duration) + ' clips' : ''}</div>
              </div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#F59E0B' }}>${estimatedCost.toFixed(2)} AUD</div>
            </div>

            <button onClick={generate} disabled={generating || stitching} style={{ padding: '14px 0', borderRadius: 12, border: 'none', cursor: (generating || stitching) ? 'wait' : 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 800, background: (generating || stitching) ? 'rgba(127,184,151,0.25)' : 'linear-gradient(135deg,#7FB897,#2D5240)', color: (generating || stitching) ? 'rgba(255,255,255,0.4)' : '#fff', letterSpacing: -0.3 }}>
              {stitching ? '🎞 Stitching clips…' : generating && totalClips > 1 ? '⏳ ' + clipProgress + '/' + totalClips + ' clips…' : generating ? '⏳ Generating…' : 'Generate ' + duration + 's Reel — $' + estimatedCost.toFixed(2) + ' AUD' + (clipsNeeded(duration) > 1 ? ' (' + clipsNeeded(duration) + ' clips)' : '')}
            </button>
          ...
          <div style={S.right}>
            {latestVideo ? (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 12, color: '#7FB897', marginBottom: 12, fontWeight: 700 }}>✅ Reel ready — switch to Edit tab</div>
      ...
      {tab === 'edit' && (
        <div style={S.body}>
          <div style={S.left}>
            {!latestVideo && <div style={{ padding: '40px 0', textAlign: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 13 }}>Generate a reel first</div>}

            <div style={S.card}>
              <span style={S.label}>Speed</span>
              <div style={S.chipRow}>
                {[0.5, 0.75, 1, 1.25, 1.5, 2].map((s) => (<button key={s} onClick={() => { setSpeed(s); if (videoRef.current) videoRef.current.playbackRate = s }} style={S.chip(speed === s)}>{s}×</button>))}
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              <div style={S.chipRow}>
                {CAPTION_STYLES.map((c) => (<button key={c.id} onClick={() => setCaptionStyle(c.id)} style={S.chip(captionStyle === c.id)}>{c.label}</button>))}
              </div>
              {captionStyle !== 'none' && (<textarea value={captionText} onChange={(e) => setCaptionText(e.target.value)} rows={2} placeholder="Caption text…" style={{ width: '100%', marginTop: 10, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontSize: 12, fontFamily: 'inherit', resize: 'none', outline: 'none', boxSizing: 'border-box' }} />)}
            </div>

            <div style={S.card}>
              <span style={S.label}>Music · Pixabay <span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 400, textTransform: 'none' }}>free commercial</span></span>
              <button onClick={() => { setMusic('none'); setSelectedTrack(null) }} style={{ ...S.chip(music === 'none'), marginBottom: 10 }}>🚫 No music</button>
              <div style={S.chipRow}>
                {MUSIC_MOODS.map((m) => (<button key={m.id} onClick={() => { setMusicMood(m.id); loadMusicTracks(m.id) }} style={S.chip(musicMood === m.id)}>{m.label}</button>))}
              </div>
              <div style={{ marginTop: 10 }}>
                {musicLoading && <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', padding: '8px 0' }}>🔍 Searching…</div>}
                {!musicLoading && musicTracks.length === 0 && (<button onClick={() => loadMusicTracks(musicMood)} style={{ fontSize: 11, color: '#7FB897', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: '4px 0' }}>Load tracks →</button>)}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 200, overflowY: 'auto' }}>
                  {musicTracks.map((t) => (
                    <div key={t.id} onClick={() => { setSelectedTrack(t); setMusic(t.url) }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 8, cursor: 'pointer', background: selectedTrack?.id === t.id ? 'rgba(127,184,151,0.12)' : 'rgba(255,255,255,0.04)', boxShadow: selectedTrack?.id === t.id ? 'inset 0 0 0 1.5px rgba(127,184,151,0.4)' : 'inset 0 0 0 1px rgba(255,255,255,0.06)', transition: 'all 120ms' }}>
                      <button onClick={(e) => { e.stopPropagation(); previewTrack(t) }} style={{ width: 24, height: 24, borderRadius: '50%', border: 'none', flexShrink: 0, background: playingId === t.id ? '#7FB897' : 'rgba(255,255,255,0.1)', color: '#fff', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{playingId === t.id ? '■' : '▶'}</button>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>{t.artist}{t.bpm ? ' · ' + t.bpm + ' BPM' : ''}</div>
                      </div>
            ...
            <div style={S.card}>
              ...
              <input value={endCard} onChange={(e) => setEndCard(e.target.value)} placeholder="End card text" style={{ width: '100%', padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontSize: 12, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
            </div>

            {latestVideo && (<a href={latestVideo} download="aria-reel.mp4" style={{ display: 'block', textAlign: 'center', padding: '13px 0', borderRadius: 12, background: 'linear-gradient(135deg,#7FB897,#2D5240)', color: '#fff', fontSize: 14, fontWeight: 800, textDecoration: 'none', letterSpacing: -0.3 }}>⬇ Download Reel</a>)}

            {latestVideo && (
              <div style={S.card}>
                <span style={S.label}>Publish to social</span>
                <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                  {(['instagram', 'facebook'] as const).map((p) => (<button key={p} onClick={() => setPublishPlatform(p)} style={{ flex: 1, padding: '8px 0', borderRadius: 9, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, fontWeight: 700, background: publishPlatform === p ? 'rgba(127,184,151,0.15)' : 'rgba(255,255,255,0.04)', boxShadow: publishPlatform === p ? 'inset 0 0 0 1.5px rgba(127,184,151,0.5)' : 'inset 0 0 0 1px rgba(255,255,255,0.06)', color: publishPlatform === p ? '#7FB897' : 'rgba(255,255,255,0.4)' }}>{p === 'instagram' ? '📸 Instagram' : '👍 Facebook'}</button>))}
                </div>
                <textarea value={publishCaption} onChange={(e) => setPublishCaption(e.target.value)} rows={3} placeholder="Caption (blank = auto)" style={{ width: '100%', padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontSize: 12, fontFamily: 'inherit', resize: 'none', outline: 'none', boxSizing: 'border-box', marginBottom: 10, lineHeight: 1.6 }} />
                <button onClick={publishReel} disabled={publishing} style={{ width: '100%', padding: '12px 0', borderRadius: 12, border: 'none', cursor: publishing ? 'wait' : 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 800, background: publishing ? 'rgba(99,102,241,0.3)' : 'linear-gradient(135deg,#6366f1,#4f46e5)', color: publishing ? 'rgba(255,255,255,0.4)' : '#fff', letterSpacing: -0.2 }}>
                  {publishing ? '⏳ Publishing…' : '🚀 Publish to ' + (publishPlatform === 'instagram' ? 'Instagram' : 'Facebook') + ' Reels'}
                </button>
                {publishMsg && <div style={{ fontSize: 12, marginTop: 8, padding: '8px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', color: publishMsg.startsWith('✅') ? '#7FB897' : '#EF4444' }}>{publishMsg}</div>}
              </div>
            )}
          </div>
              ...
              <div key={s.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '10px 12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'capitalize', color: s.status === 'completed' ? '#7FB897' : s.status === 'failed' ? '#EF4444' : '#F59E0B' }}>{s.status}</span>
      ...
      {showPicker && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setShowPicker(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 640, maxHeight: '88vh', overflowY: 'auto', background: '#0f1117', borderRadius: '20px 20px 0 0', padding: 24 }}>
            ...
            {influencers.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>Loading influencers…</div>
            ) : (
