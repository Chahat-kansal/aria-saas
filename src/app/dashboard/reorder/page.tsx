...
function downloadCSV(items: ForecastItem[]) {
  ...
  const rows = items.map(i => ({
    ...
    'Suggested Order': i.suggested_order,
    Unit: i.unit,
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p style={{ color: '#6b7280' }}>Forecast based on connected sales velocity, stock and real calendar signals.</p>
      ...
      {forecast?.no_data && (
        <div className="rounded-xl p-12 text-center" style={{ background: '#13131a', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <p className="text-xs" style={{ color: '#4b5563' }}>Smart ordering needs sales and inventory data.</p>
        </div>
      )}
        ...
        <>
          ...
          {forecast.items.length === 0 ? (
            <div className="rounded-xl p-12 text-center" style={{ background: '#13131a', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <p className="text-sm" style={{ color: '#6b7280' }}>No orders needed from the connected forecast right now.</p>
            </div>
          ) : (
            <div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: '#13131a', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                      {['Product', 'Stock', 'Daily Sales', 'Days Left', 'Calendar Impact', 'Order Qty', 'Order By', 'Urgency'].map(h => (
                      ...
                      return (
                        <tr key={item.item_id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                          ...
                          <td className="px-3 py-3 text-xs" style={{ color: item.holiday_uplift > 1 ? '#f59e0b' : '#4b5563' }}>
                            {item.holiday_uplift > 1 ? `x${item.holiday_uplift.toFixed(1)}` : '-'}
                          </td>
