...
function downloadCSV(items: VItem[]) {
  const rows = items.map(i => ({ Product:i.item_name, Expected:i.theoretical_stock, Actual:i.actual_stock, Variance:i.variance, 'Value Gap (A$)':(i.variance_value_cents/100).toFixed(2), '%':i.variance_pct }));
...
export default function VariancePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/aria/variance', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({business_id:business.id}) });
  ...
  // Build chart when items load and view is chart
  useEffect(() => {
    ...
    function buildChart() {
      ...
      const sorted = [...items].sort((a,b) => a.variance_value_cents - b.variance_value_cents).slice(0,15);
      chartInstance.current = new ChartJS(chartRef.current!, {
        type: 'bar',
        data: {
          labels: sorted.map(i => i.item_name.length > 20 ? i.item_name.slice(0,18)+'…' : i.item_name),
          datasets: [{
            label: 'Shrinkage Value (A$)',
            data: sorted.map(i => -(i.variance_value_cents/100)),
            backgroundColor: sorted.map(i => i.variance < 0 ? 'rgba(239,68,68,0.7)' : 'rgba(34,197,94,0.7)'),
            borderRadius: 4,
          }]
        },
        options: {
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p style={{ color:'#6b7280' }}>Gap between expected and actual stock. Top 15 worst items shown in chart.</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {(['chart','table'] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              className="text-xs px-3 py-2 rounded-xl transition-colors capitalize"
              style={{ background: view===v ? 'rgba(139,92,246,0.15)' : 'rgba(255,255,255,0.04)', color: view===v ? '#8B5CF6' : 'rgba(255,255,255,0.4)', border: view===v ? '1px solid rgba(139,92,246,0.3)' : '1px solid rgba(255,255,255,0.08)' }}>
              {v}
            </button>
          ))}
          {items.length > 0 && (
      ...
      {loading && items.length === 0 && (
        <div className="animate-pulse space-y-2">{[...Array(5)].map((_,i)=><div key={i} className="h-12 rounded-xl bg-[rgba(255,255,255,0.04)]" />)}</div>
      )}
      ...
      {!loading && items.length === 0 && !error && (
        <div className="rounded-xl p-12 text-center" style={{ background:'#13131a', border:'1px solid rgba(255,255,255,0.07)' }}>
          ...
          <p className="text-sm" style={{ color:'#6b7280' }}>Your stock levels are accurate. Great work!</p>
        </div>
      )}
      ...
      {items.length > 0 && (
        <>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="rounded-xl p-4" style={{ background:totalShrinkage>0?'rgba(239,68,68,0.1)':'#13131a', border:totalShrinkage>0?'1px solid rgba(239,68,68,0.2)':'1px solid rgba(255,255,255,0.07)' }}>
              <p className="text-xs mb-1" style={{ color:'#6b7280' }}>Total shrinkage value</p>
              <p className="text-2xl font-semibold" style={{ color:totalShrinkage>0?'#ef4444':'#fff' }}>A${(totalShrinkage/100).toFixed(2)}</p>
              <p className="text-xs mt-1" style={{ color:'#6b7280' }}>Estimated stock loss</p>
            </div>
            <div className="rounded-xl p-4" style={{ background:'#13131a', border:'1px solid rgba(255,255,255,0.07)' }}>
              <p className="text-xs mb-1" style={{ color:'#6b7280' }}>Items with shrinkage</p>
              <p className="text-2xl font-semibold text-red-400">{lossItems.length}</p>
              <p className="text-xs mt-1" style={{ color:'#6b7280' }}>Negative variance</p>
            </div>
            <div className="rounded-xl p-4" style={{ background:'#13131a', border:'1px solid rgba(255,255,255,0.07)' }}>
              ...
              <p className="text-xs mt-1" style={{ color:'#6b7280' }}>Positive variance</p>
            </div>
          </div>

          {view === 'chart' ? (
            <div style={{ position:'relative', height:Math.max(300, Math.min(items.length,15)*36+80), background:'#13131a', borderRadius:12, border:'1px solid rgba(255,255,255,0.07)', padding:'16px' }}>
              <canvas ref={chartRef} role="img" aria-label="Horizontal bar chart showing shrinkage value by product" />
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody style={{ background:'#0d0d14' }}>
                  {items.map(item => {
                    ...
                    return (
                      <tr key={item.item_id} className="cursor-pointer hover:bg-[rgba(255,255,255,0.02)] transition-colors"
          ...
          <p className="text-xs mt-3" style={{ color:'rgba(255,255,255,0.2)' }}>Based on stock movements and sales in the last 30 days. Click any row for full AI insight.</p>
        </>
      )}
    </div>
  );
