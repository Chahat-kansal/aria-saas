...
export default function AuditChecksPage() {
  ...
  const submit = async () => {
    ...
    const results = templates.map(t => ({ template_id: t.id, name: t.name, category: t.category, required: t.required, passed: answers[t.id] ?? false, note: notes[t.id] ?? '' }))
    try {
      ...
      if (d.ok) { setMsg('Audit submitted!'); setAnswers({}); setNotes({}); setTab('history'); load() } else setMsg(d.error ?? 'Failed')
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                {templates.map(t => (
                  <div key={t.id} style={{ background: S.surface, border: '1px solid ' + (answers[t.id] === false && t.required ? 'rgba(239,68,68,0.3)' : S.border), borderRadius: 12, padding: '14px 18px' }}>
                    ...
                    {answers[t.id] === false && <input placeholder="Note (optional)" value={notes[t.id] ?? ''} onChange={e => setNotes(prev => ({ ...prev, [t.id]: e.target.value }))} style={{ marginTop: 10, width: '100%', padding: '7px 10px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 7, color: '#e8ede7', fontSize: 12, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />}
                  </div>
                ))}
              </div>
