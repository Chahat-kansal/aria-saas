...
function ABN({ form, set, abnState, onABNBlur }: { form: FD; set: Setter; abnState: 'valid' | 'invalid' | 'empty'; onABNBlur: () => void }) {
  ...
  const [verifyResult, setVerifyResult] = React.useState<{ entity_name?: string; gst_registered?: boolean; abn_status?: string; active?: boolean } | null>(null);
  ...
  async function verifyABN() {
    ...
    try {
      ...
      if (!res.ok) { setVerifyError(data.error ?? 'Lookup failed'); return; }
      if (!data.found) { setVerifyError('ABN not found in Australian Business Register'); return; }
      setVerifyResult(data);
      // Auto-fill fields from ABR
        ...
        {verifyResult && (
          <div className="mt-2 p-2.5 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-xs font-semibold text-green-700 mb-0.5">
              ✓ Verified with Australian Business Register
              {!verifyResult.active && ' — Note: ABN status is not Active'}
            </p>
            {verifyResult.entity_name && <p className="text-xs text-green-600">Registered name: <strong>{verifyResult.entity_name}</strong></p>}
            {verifyResult.gst_registered !== undefined && (
              <p className="text-xs text-green-600">
                GST: {verifyResult.gst_registered ? '✓ Registered' : '✗ Not registered'} — prefilled below
              </p>
            )}
          </div>
        )}
