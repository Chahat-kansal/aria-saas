...
const PLAN_LIMITS = {
  ...
  pro:   { messages: Infinity, models: ['claude-haiku-4-5-20251001','claude-sonnet-4-20250514','claude-opus-4-20250514'], webSearch: true },
};
...
export async function POST(req: Request) {
  ...
  if (user.messagesUsedThisMonth >= plan.messages) {
    return new Response(JSON.stringify({ error: 'Monthly message limit reached. Upgrade to Pro for unlimited messages.' }), { status: 429 });
  }

  const { message, conversationId, model, useWebSearch, system } = await req.json();

  if (!message?.trim()) return new Response(JSON.stringify({ error: 'Message required' }), { status: 400 });

  const safeModel = ALLOWED_MODELS.has(model) && plan.models.includes(model)
  ...
  // Build messages for Claude (last 40)
  ...
  const systemPrompt = system?.trim() ||
    'You are Aria, a friendly and highly capable AI assistant. You are helpful, thoughtful, and concise. When using web search, always cite your sources with links.';
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Save assistant reply + update usage
