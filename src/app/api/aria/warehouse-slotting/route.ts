...
async function _POST(req: Request) {
  ...
  const msg = 
await trackAICall({ route: 'aria/warehouse-slotting', model: 'claude-sonnet-4-5-20250929', businessId: business_id, purpose: 'warehouse-slotting' }, () => anthropic.messages.create({
...
Zones available: ${JSON.stringify([...new Set((locations.data ?? []).map((l: any) => l.zone))])}`,
    ...
    const suggestions = parseLLMJsonOr<any[]>(raw, [], 'aria/warehouse-slotting');
