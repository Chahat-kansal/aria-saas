...
const PROXY = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/higgsfield-proxy`
...
async function callHF(endpoint: string, payload?: object, method = 'POST') {
  ...
  if (!res.ok) {
    if (text.includes('HIGGSFIELD_API_KEY not configured'))
      throw new Error('Higgsfield API key not set in Supabase secrets. Go to Supabase dashboard → Project Settings → Edge Functions → Add HIGGSFIELD_API_KEY secret.')
...
export async function POST(req: NextRequest) {
  ...
  const { business_id, influencer_id, soul_id, higgsfield_job_id, scene_image_url,
    prompt, style = 'lifestyle', duration_seconds = 10, resolution = '720p', mode = 'std' } = await req.json()
  ...
  const costAud = Math.round((dur <= 10 ? 10 : 20) * 0.095 * 100) / 100
  const videoPrompt = ((STYLE_PROMPTS[style] ?? STYLE_PROMPTS.lifestyle) + ' ' + (prompt ?? 'Australian small business, authentic and warm') + ', 9:16 vertical').slice(0, 500)

  const genPayload: Record<string, any> = { model: 'kling3_0', prompt: videoPrompt, aspect_ratio: '9:16', duration: dur, resolution, mode, sound: 'on' }
  if (soul_id) genPayload.soul_id = soul_id
  if (higgsfield_job_id) { genPayload.medias = [{ value: higgsfield_job_id, role: 'start_image' }]; if (scene_image_url) genPayload.medias.push({ value: scene_image_url, role: 'end_image' }) }
  ...
  const { data: session } = await supabaseAdmin.from('reel_studio_sessions').insert({
    ...
    scene_image_url: scene_image_url ?? null, prompt: videoPrompt, style,
    duration_seconds: dur, status: 'processing', cost_aud: costAud, credits_used: dur <= 10 ? 10 : 20,
  }).select().single()

  try {
    const d = await callHF('/v1/video/generate', genPayload)
    const jobId = d.id ?? d.job_id ?? d.request_id
    ...
    return NextResponse.json({ job_id: jobId, session_id: session?.id, duration: dur, estimated_cost_aud: costAud, status: 'queued' })
  } catch (e: any) {
