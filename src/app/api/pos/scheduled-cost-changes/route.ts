...
async function _GET(_req: Request) {
  ...
  const { data } = await supabase.from('pos_scheduled_cost_changes')
    .select('*, pos_products(name, cost_price)')
...
async function _DELETE(req: Request) {
  ...
  await supabase.from('pos_scheduled_cost_changes').delete().eq('id', id).eq('business_id', bid ?? '')
