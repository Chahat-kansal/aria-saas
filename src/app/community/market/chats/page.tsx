...
interface BizRef { name: string | null; logo_url: string | null; community_verified: boolean | null }
interface ChatRow {
  id: string
  ...
  businesses: BizRef | null
  marketplace_listings: { title: string; price: number | null; media_urls: string[]; status: string; businesses: BizRef | null } | null
}
...
export default function MyChatsPage() {
  ...
  // Business DMs have listing_id IS NULL (no listing to render); marketplace chats have a listing.
  const dmChats = chats.filter(c => !c.listing_id)
  const listingChats = chats.filter(c => c.listing_id)
    ...
    <main style={{ maxWidth: MAX_W, margin: '0 auto', padding: '20px 16px 24px' }}>
      ...
      {!member && !loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          {/* Direct messages — business DMs (listing_id IS NULL) */}
          {dmChats.length > 0 && (
            <section>
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.accent, margin: '0 0 10px' }}>Direct messages</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {dmChats.map(c => {
                  const last = c.messages?.[c.messages.length - 1]
                  const biz = c.businesses
                  return (
                    <Link key={c.id} href={`/community/dm/${c.business_id}`} prefetch={false} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 700, margin: 0, display: 'inline-flex', alignItems: 'center', gap: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>
                          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{biz?.name ?? 'Shop'}</span>
                          {biz?.community_verified && <BadgeCheck size={12} style={{ color: C.accent, flexShrink: 0 }} />}
                        </p>
                        <p style={{ fontSize: 12, color: C.textMuted, margin: '2px 0 0' }}>Direct message</p>
                        {last && (
          ...
          {/* Marketplace — listing chats (unchanged) */}
          {listingChats.length > 0 && (
            <section>
              {dmChats.length > 0 && <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.textMuted, margin: '0 0 10px' }}>Marketplace</p>}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {listingChats.map(c => {
                  const last = c.messages?.[c.messages.length - 1]
