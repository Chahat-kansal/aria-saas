...
// CX-CLARITY-1 — extracted verbatim from what used to be /community/page.tsx (same feed, same
// behaviour) so the new /community/page.tsx (a server component) can do the café-first redirect/
// picker check BEFORE this ever renders. By construction this now only renders for a visitor with
// no session or zero linked cafés — reframed accordingly (header retitled "Discover" + subtitle;
...
export function DiscoverFeed() {
  ...
  return (
    <main style={{ maxWidth: MAX_W, margin: '0 auto', padding: '20px 16px 24px' }}>
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: ownerBusiness ? 12 : 4, gap: 8 }}>
        {/* CX-CLARITY-1 — retitled "Discover" (was the "a. aria" wordmark feed header). This page
            is now only ever reached with no session / zero linked cafés (see /community/page.tsx's
            server-side entry check) — a linked member never sees this as their front door anymore. */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
          ...
          <span style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.03em', color: PALETTE.ink }}>Discover</span>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
          <Link href="/community/discover" prefetch={false} aria-label="Discover directory" style={roundBtn}>
      ...
      <p style={{ fontSize: 11, color: PALETTE.inkSoft, margin: '0 0 16px', fontWeight: 500 }}>
        Communities from local businesses on Aria.
      </p>
