...
import { BadgeCheck, ExternalLink, Bell, BellOff, EyeOff, Eye, Loader2, Compass, MoreHorizontal, X, Phone, MapPin } from 'lucide-react'
import { PALETTE, BORDER, RADIUS, MAX_W } from '../theme'
import { MemberStatusCard, type YourStatus } from '../MemberStatusCard'
import { LoopExplainerBanner } from '../LoopExplainerBanner'
import { LeaderboardEntryCard } from '../LeaderboardEntryCard'

// CX-CLARITY-1 — the café page IS the real home now (see /community/page.tsx's café-first entry
// logic). This is the same business-scoped profile+feed that used to live at
// /community/businesses/[id] (moved here so it can be addressed by slug) — reused wholesale, not
// rebuilt, plus the comprehension layer this sprint adds: MemberStatusCard, the loop explainer, the
// leaderboard entry card, and a real empty-feed explainer instead of a blank "no posts yet."
...
export default function CafePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [pRes, fRes, linkRes] = await Promise.all([
        fetch('/api/community/businesses/' + slug + '/profile').then(r => r.ok ? r.json() : null).catch(() => null),
        fetch('/api/community/follows').then(r => r.json()).catch(() => ({ follows: [] })),
      ...
      setProfile(pRes)
      const f = (fRes.follows ?? []).find((x: { business_id: string }) => x.business_id === pRes.business.id)
      if (f) {
  ...
  useEffect(() => {
    if (tab === 'reels' && reels === null && profile?.business.id) {
      fetch('/api/community/reels?business_id=' + profile.business.id).then(r => r.ok ? r.json() : { reels: [] })
        .then(d => setReels(d.reels ?? [])).catch(() => setReels([]))
    }
  }, [tab, reels, profile?.business.id])
  ...
  async function follow() {
    ...
    await fetch('/api/community/follows', {
      ...
      body: JSON.stringify({ business_id: profile.business.id, consent_marketing: consentMarketing, notifications_on: notificationsOn, nickname: nickname || undefined }),
    })
  ...
  async function unfollow() {
    ...
    setBusy(true)
    await fetch('/api/community/follows?business_id=' + profile.business.id, { method: 'DELETE' })
    setBusy(false); load()
  }
  async function togglePref(key: 'notifications_on' | 'is_hidden', value: boolean) {
    ...
    await fetch('/api/community/follows', {
      ...
      body: JSON.stringify({ business_id: profile.business.id, [key]: value }),
    })
  }
  ...
  if (notFound || !profile) {
    return (
      <main style={{ maxWidth: MAX_W, margin: '0 auto', padding: '60px 20px', textAlign: 'center' }}>
        ...
        <Link href="/community/discover" style={{ display: 'inline-block', marginTop: 20, color: PALETTE.ink, fontWeight: 700 }}>← discover cafés</Link>
      </main>
    )
  }
  ...
  return (
    <main style={{ maxWidth: MAX_W, margin: '0 auto', padding: '12px 16px 24px' }}>
      {/* Top bar — Discover link (nothing deleted, just a new path back to the global feed) + menu */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <Link href="/community/discover" prefetch={false} aria-label="Discover" style={iconRound}>
          <Compass size={16} color={PALETTE.ink} />
        </Link>
      ...
      <LoopExplainerBanner slug={slug} businessName={biz.name} linked={!!profile.your_status} initialDismissed={profile.banner_dismissed} />
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: -32, padding: '0 4px', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <Link href={`/community/dm/${biz.id}`} prefetch={false} style={whitePill}>message</Link>
        </div>
      </div>
      ...
      {/* CX-CLARITY-1 — the comprehension anchor, on every business-scoped screen. */}
      <MemberStatusCard yourStatus={profile.your_status} slug={slug} businessName={biz.name} />

      {/* CX-CLARITY-1 — makes the leaderboard findable instead of an unfindable sub-route. */}
      <LeaderboardEntryCard slug={slug} />
      ...
      <div style={{ marginTop: 14 }}>
        {tab === 'posts' ? (
          profile.recent_posts.length === 0 ? (
            // CX-CLARITY-1 — cold-start comprehension: an empty café community still explains
            // itself instead of a blank "no posts yet." The leaderboard/status cards above already
            // cover the loop; this just reinforces it in the feed slot itself.
            <div style={{ textAlign: 'center', padding: 28 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: PALETTE.ink, margin: '0 0 6px' }}>No posts yet — but the loop still works</p>
              <p style={{ fontSize: 12, color: PALETTE.inkSoft, lineHeight: 1.5, maxWidth: 320, margin: '0 auto' }}>
                Every visit earns points here. Points level you up automatically, and real rewards unlock along the way — check your status above.
              </p>
            </div>
          ) : (
