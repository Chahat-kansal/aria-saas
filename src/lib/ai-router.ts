...
const SYSTEM_PROMPTS: Record<AiTask, string> = {
  ...
  insight: `You are Aria, a business event intelligence engine for Australian small businesses. Observe a business event and generate ONE specific, actionable insight. ${EVIDENCE_RULES} ${AUSTRALIAN_CONTEXT} Respond with valid JSON only. No prose before or after. No code fences. Schema: {"title":"max 8 words","description":"max 25 words with specific number","priority":"critical|high|medium|low","estimated_impact":"A$ estimate or qualitative","suggested_action":"one specific action"}`,
}
