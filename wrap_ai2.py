    ...
    while i < len(text):
        ...
        if in_str:
            if c == in_str:
                ...
                pass  # placeholder — actual backslash handling below
        else:
            if c in ('"', "'", '`'):
                in_str = c
            elif c == '(':
...
def wrap_file(file_path):
    ...
    call_re = re.compile(re.escape(client_var) + r'\.messages\.create\(')
    ...
    for i, m in enumerate(reversed(matches)):
        ...
        ctx = "{ route: '" + route + "', model: '" + model + "', businessId: " + biz_id + ", purpose: '" + purpose + "' }"
        replacement = "trackAICall(" + ctx + ", () => " + client_var + ".messages.create" + call_text + ")"
    ...
    with open(file_path, 'w', encoding='utf-8', newline='\n') as f:
        f.write(content)
    return True, 'wrapped ' + str(len(matches)) + ' call(s)'
...
for root, _, files in os.walk(base):
    for file in files:
        ...
        try:
            ...
            sym = 'OK' if ok else '--'
            print(sym + '  ' + get_route_name(fp) + ': ' + reason)
            if ok:
                done += 1
            else:
                skip += 1
        except Exception as e:
            print('ERR ' + get_route_name(fp) + ': ' + str(e))
            err += 1
print('\nwrapped=' + str(done) + ' skipped=' + str(skip) + ' errors=' + str(err))
